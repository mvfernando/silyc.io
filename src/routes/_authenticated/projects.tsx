...
import { Plus, FileVideo, Loader2 } from "lucide-react";
      ...
      <main className="mx-auto max-w-5xl px-4 py-10">
        <div className="flex items-center justify-between">
          ...
          <Button asChild className="gap-2">
            <Link to="/app">
              <Plus className="h-4 w-4" /> {t.proj_new}
            </Link>
          </Button>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="mt-12 rounded-xl border border-dashed border-border bg-card/30 p-12 text-center text-muted-foreground">
            <FileVideo className="mx-auto h-8 w-8 opacity-60" />
            <p className="mt-3 text-sm">{t.proj_empty}</p>
              ...
              return (
                ...
                >
                  <Link
                    to="/projects/$id"
                    params={{ id: p.id }}
                    className="flex items-center justify-between rounded-xl border border-border/80 bg-card/40 p-4 transition-colors hover:bg-card/70"
                  >
                    <div>

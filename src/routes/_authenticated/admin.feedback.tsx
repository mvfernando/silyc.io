...
export const Route = createFileRoute("/_authenticated/admin/feedback")({
  head: () => ({ meta: [{ title: "Silyc — Admin · Feedback" }] }),
...
function AdminFeedbackPage() {
  ...
  const { data: rows, isLoading } = useQuery({
    ...
    enabled: !!isAdmin,
    queryFn: async (): Promise<FeedbackRow[]> => await fetchFeedback({ data: { since: periodStart(period) } }),
  });
  ...
  return (
    <AdminShell
      ...
      description="Reactions, refinement choices and rework rate across agent runs."
      actions={
        <>
          <Segmented
            value={period}
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <Kpi label="Top format" value={topKey(stats.formatCounts) ?? "—"} />
          </div>
        ...
        return (
          <li key={i.key}>
            ...
            <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-muted/50">
              <div className="h-full bg-foreground/70" style={{ width: `${pctOfMax}%` }} />
            </div>
          </li>
        );
      })}
